...
def pretty_print(element, indent):
    nspace = 2
    if list(element):
        ret = '{}<{}>\n'.format(' ' * indent, element.tag)
        for e in element:
            ret += pretty_print(e, indent + nspace)
        if element.text:
            ret += '{}{}\n'.format(' ' * (indent + nspace), element.text)
        ret += '{}</{}>\n'.format(' ' * indent, element.tag)
    else:
        ret = '{}<{}>{}</{}>\n'.format(' ' * indent, element.tag, element.text, element.tag)
    return ret
...
if __name__ == '__main__':
    ...
    for v in sys.argv[1:]:
        ...
        with open(v, 'rb') as f:
            try:
                root = xmlunpacker.read(f)
            except:
                print('maybe not XML file: {}'.format(v))
                continue

        #root.tag = os.path.basename(v)
        ...
        try:
            ...
            w = ET.tostring(root)
            mroot = etree.fromstring(w)
            s = etree.tostring(mroot, encoding='utf-8', pretty_print=True)
        except:
            s = pretty_print(root, 0).encode('utf-8')
